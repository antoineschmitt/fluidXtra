...
enum /* Here is the list of known format tags */
{	
...
{	WAV_FMT			wav_fmt ;
	...
	{	fread (&marker, sizeof (marker), 1, psf->file) ;
		...
		{	case RIFF_MARKER :
					...
					{	dword = psf->filelength - 2 * sizeof (dword);
						__psf_sprintf (psf, "RIFF : %d (should be %d)\n", RIFFsize, dword) ;
						RIFFsize = dword ;
						}
					else
						__psf_sprintf (psf, "RIFF : %d\n", RIFFsize) ;
					parsestage = 1 ;
					break ;
			...
			case data_MARKER :
					...
					{	__psf_sprintf (psf, "data : %d (should be %d)\n", psf->datalength, psf->filelength - psf->dataoffset) ;
						psf->datalength = psf->filelength - psf->dataoffset ;
						}
					else
						__psf_sprintf (psf, "data : %d\n", psf->datalength) ;

					if (format == WAVE_FORMAT_MS_ADPCM && psf->datalength % 2)
					{	psf->datalength ++ ;
						__psf_sprintf (psf, "*** Data length odd. Increasing it by 1.\n") ;
						} ;
		
					fseek (psf->file, (int) psf->datalength, SEEK_CUR) ;
			...
			case fact_MARKER :
					...
					__psf_sprintf (psf, "%D : %d\n", marker, dword) ;
					__psf_sprintf (psf, "  samples : %d\n", fact_chunk.samples) ;
					break ;
			...
			default : 
					...
					{	fread (&dword, sizeof (dword), 1, psf->file) ;
						__psf_sprintf (psf, "%D : %d (unknown marker)\n", marker, dword) ;
						fseek (psf->file, (int) dword, SEEK_CUR) ;
						break ;
						} ;
					if ((dword = ftell (psf->file)) & 0x03)
					{	__psf_sprintf (psf, "  Unknown chunk marker at position %d. Resynching.\n", dword) ;
						fseek (psf->file, -3, SEEK_CUR) ;
						break ;
						} ;
					__psf_sprintf (psf, "*** Unknown chunk marker : %X. Exiting parser.\n", marker) ;
					break ;
			} ;	/* switch (dword) */

		if (ferror (psf->file))
		{	__psf_sprintf (psf, "*** Error on file handle. ***\n", marker) ;
			clearerr (psf->file) ;
			break ;
			} ;
	...
	fseek (psf->file, (int) psf->dataoffset, SEEK_SET) ;
	
	psf->close = (func_close) __wav_close ;
	...
	{	case 	WAVE_FORMAT_PCM :
		...
		case	WAVE_FORMAT_MS_ADPCM : 
					...
					if ((error = __msadpcm_reader_init (psf, &wav_fmt)))
						return error ;
					psf->read_short  = (func_short)  __msadpcm_read_s ;
					psf->read_int    = (func_int)    __msadpcm_read_i ;
					psf->read_double = (func_double) __msadpcm_read_d ;
					psf->seek_func   = (func_seek)   __msadpcm_seek ;
					return 0 ;

		case	WAVE_FORMAT_IMA_ADPCM :
					psf->sf.format   = (SF_FORMAT_WAV | SF_FORMAT_IMA_ADPCM) ;
					if ((error = __ima_reader_init (psf, &wav_fmt)))
						return error ;
					psf->read_short  = (func_short)  __ima_read_s ;
					psf->read_int    = (func_int)    __ima_read_i ;
					psf->read_double = (func_double) __ima_read_d ;
					psf->seek_func   = (func_seek)   __ima_seek ;
					return 0 ;
		
		case	WAVE_FORMAT_IEEE_FLOAT :
					psf->sf.format   = (SF_FORMAT_WAV | SF_FORMAT_FLOAT) ;
					if (CAN_READ_WRITE_x86_IEEE)
					{	psf->read_short  = (func_short)  __pcm_read_f2s ;
						psf->read_int    = (func_int)    __pcm_read_f2i ;
						psf->read_double = (func_double) __pcm_read_f2d ;
						}
					else 
					{	psf->read_short  = (func_short)  __wav_read_x86f2s ;
						psf->read_int    = (func_int)    __wav_read_x86f2i ;
						psf->read_double = (func_double) __wav_read_x86f2d ;
						} ;
					return 0 ;
	...
	{	case  1 :
				psf->read_short  = (func_short)  __pcm_read_uc2s ;
				psf->read_int    = (func_int)    __pcm_read_uc2i ;
				psf->read_double = (func_double) __pcm_read_uc2d ;
				break ;
		case  2 :
...
{	WAV_FMT			wav_fmt ;
	...
	{	case	SF_FORMAT_PCM : 
		...
		case	SF_FORMAT_MS_ADPCM : 
					...
					__msadpcm_writer_init (psf, &wav_fmt) ;
	...
	{	psf->sf.format = (SF_FORMAT_WAV | SF_FORMAT_FLOAT) ;
		if (CAN_READ_WRITE_x86_IEEE)
		{	psf->write_short  = (func_short)  __pcm_write_s2f ;
			psf->write_int    = (func_int)    __pcm_write_i2f ;
			psf->write_double = (func_double) __pcm_write_d2f ;
			}
		else 
		{	psf->write_short  = (func_short)  __wav_write_s2x86f ;
			psf->write_int    = (func_int)    __wav_write_i2x86f ;
			psf->write_double = (func_double) __wav_write_d2x86f ;
			} ;
		return	 0 ;
		} ;
	...
	{	psf->sf.format    = (SF_FORMAT_WAV | SF_FORMAT_IMA_ADPCM) ;
		psf->write_short  = (func_short)  __ima_write_s ;
		psf->write_int    = (func_int)    __ima_write_i ;
		psf->write_double = (func_double) __ima_write_d ;
		psf->seek_func    = (func_seek)   __ima_seek ;
		psf->close        = (func_close)  __ima_close ;
		return 0 ;
		} ;

	if ((psf->sf.format & SF_FORMAT_SUBMASK) == SF_FORMAT_MS_ADPCM)
	{	psf->sf.format    = (SF_FORMAT_WAV | SF_FORMAT_MS_ADPCM) ;
		psf->write_short  = (func_short)  __msadpcm_write_s ;
		psf->write_int    = (func_int)    __msadpcm_write_i ;
		psf->write_double = (func_double) __msadpcm_write_d ;
		psf->seek_func    = (func_seek)   __msadpcm_seek ;
		psf->close        = (func_close)  __msadpcm_close ;
		return 0 ;
		} ;
	...
	switch (psf->bytewidth)
	{	case  1 :
				psf->write_short  = (func_short)  __pcm_write_s2uc ;
				psf->write_int    = (func_int)    __pcm_write_i2uc ;
				psf->write_double = (func_double) __pcm_write_d2uc ;
				break ;
		case  2 :
...
{	unsigned int	dword, bytesread, k, structsize  ;
	...
	{	case WAVE_FORMAT_PCM :
		...
		case WAVE_FORMAT_MULAW :
				...
				psf->bytewidth      = 1 ;
				if (structsize >= 18)
					__psf_sprintf (psf, "  Extra Bytes   : %d\n", wav_fmt->size20.extrabytes) ;
				break ;
		...
		case WAVE_FORMAT_EXTENSIBLE :
				__psf_sprintf (psf, "  Valid Bits    : %d\n", wav_fmt->ext.validbits) ;
				__psf_sprintf (psf, "  Channel Mask  : 0x%X\n", wav_fmt->ext.channelmask) ;
				__psf_sprintf (psf, "  Subformat\n") ;
				__psf_sprintf (psf, "    esf_field1 : 0x%X\n", wav_fmt->ext.esf.esf_field1) ;
				__psf_sprintf (psf, "    esf_field2 : 0x%X\n", wav_fmt->ext.esf.esf_field2) ;
				__psf_sprintf (psf, "    esf_field3 : 0x%X\n", wav_fmt->ext.esf.esf_field3) ;
				__psf_sprintf (psf, "    esf_field4 : ") ;
				for (k = 0 ; k < 8 ; k++)
					__psf_sprintf (psf, "0x%X ", wav_fmt->ext.esf.esf_field4 [k] & 0xFF) ;
				__psf_sprintf (psf, "\n") ;
