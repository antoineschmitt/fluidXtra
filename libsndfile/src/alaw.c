...
int		__alaw_read_alaw2i (SF_PRIVATE *psf, int *ptr, unsigned int len)
{	unsigned int readcount, thisread, index = 0 ;
	int		bytecount, bufferlen ;
	int		total = 0 ;
...
int		__alaw_read_alaw2d (SF_PRIVATE *psf, double *ptr, unsigned int len, int normalize)
{	unsigned int readcount, thisread, index = 0 ;
	int		bytecount, bufferlen ;
	int		total = 0 ;
	double	normfact ;
...
int	__alaw_write_i2alaw	(SF_PRIVATE *psf, int *ptr, unsigned int len)
{	unsigned int	writecount, thiswrite, index = 0 ;
	int		bytecount, bufferlen ;
	int		total = 0 ;
...
int	__alaw_write_d2alaw	(SF_PRIVATE *psf, double *ptr, unsigned int len, int normalize)
{	unsigned int	writecount, thiswrite, index = 0 ;
	int		bytecount, bufferlen ;
	int		total = 0 ;
	double	normfact ;
...
{	unsigned int	k ;
	...
	for (k = 0 ; k < count ; k++)
	{	if (ptr [index] >= 0) 
			buffer [k] = alaw_encode [((int) ptr [index]) / 16] ;
		else
			buffer [k] = 0x7F & alaw_encode [((int) ptr [index]) / -16] ;
		index ++ ;
		} ;
} /* d2alaw_array */
