...
int		__ulaw_read_ulaw2i (SF_PRIVATE *psf, int *ptr, unsigned int len)
{	unsigned int readcount, thisread, index = 0 ;
	int		bytecount, bufferlen ;
	int		total = 0 ;
...
int		__ulaw_read_ulaw2d (SF_PRIVATE *psf, double *ptr, unsigned int len, int normalize)
{	unsigned int readcount, thisread, index = 0 ;
	int		bytecount, bufferlen ;
	int		total = 0 ;
	double	normfact ;
...
int	__ulaw_write_i2ulaw	(SF_PRIVATE *psf, int *ptr, unsigned int len)
{	unsigned int	writecount, thiswrite, index = 0 ;
	int		bytecount, bufferlen ;
	int		total = 0 ;
...
int	__ulaw_write_d2ulaw	(SF_PRIVATE *psf, double *ptr, unsigned int len, int normalize)
{	unsigned int	writecount, thiswrite, index = 0 ;
	int		bytecount, bufferlen ;
	int		total = 0 ;
	double	normfact ;
...
{	unsigned int	k ;
	...
	for (k = 0 ; k < count ; k++)
	{	if (ptr [index] >= 0) 
			buffer [k] = ulaw_encode [((int) ptr [index]) / 4] ;
		else
			buffer [k] = 0x7F & ulaw_encode [((int) ptr [index]) / -4] ;
		index ++ ;
		} ;
} /* d2ulaw_array */
