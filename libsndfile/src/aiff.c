...
static	int		__aiff_close	(SF_PRIVATE  *psf) ;
...
{	COMM_CHUNK			comm_fmt ;
	SSND_CHUNK			ssnd_fmt ;
	unsigned int		FORMsize, commsize, dword, marker ;
	int					filetype, parsestage = 0 ;
	...
	{	fread (&marker, sizeof (marker), 1, psf->file) ;
		...
		{	case FORM_MARKER :
					...
					{	dword = psf->filelength - 2 * sizeof (dword);
						__psf_sprintf (psf, "FORM : %d (should be %d)\n", FORMsize, dword) ;
						FORMsize = dword ;
						}
					else
						__psf_sprintf (psf, "FORM : %d\n", FORMsize) ;
					parsestage = 1 ;
					break ;
			...
			case SSND_MARKER :
					...
					{	__psf_sprintf (psf, " SSND : %d (should be %d)\n", psf->datalength, psf->filelength - psf->dataoffset) ;
						psf->datalength = psf->filelength - psf->dataoffset ;
						} 
					else
						__psf_sprintf (psf, " SSND : %d\n", psf->datalength) ;
					__psf_sprintf (psf, "  Offset     : %d\n", ssnd_fmt.offset) ;
					__psf_sprintf (psf, "  Block Size : %d\n", ssnd_fmt.blocksize) ;
	...
	psf->close = (func_close) __aiff_close ;
...
{	COMM_CHUNK			comm_fmt ;
	...
	FORMsize   = psf->filelength - 2 * sizeof (dword) ;
...
int	__aiff_close	(SF_PRIVATE  *psf)
{	unsigned int	dword ;
	...
	{	/*  Now we know for certain the length of the file we can re-write 
		...
		*/
		...
		fseek (psf->file, (int) (psf->dataoffset - (sizeof (SSND_CHUNK) + sizeof (dword))), SEEK_SET) ;
		psf->datalength = psf->filelength - psf->dataoffset ;
 ...
 *	rate.
 *	There is another (probably better) version in the source code to the SoX but
 *	it has a copyright which rpobably prevents it from being allowable as GPL.
 */
