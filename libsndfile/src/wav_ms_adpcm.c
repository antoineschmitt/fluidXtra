...
{	MSADPCM_PRIVATE	*pms ;
	...
	__psf_sprintf (psf, " bpred   idelta\n") ;
...
{	int		chan, k, blockindex, sampleindex ;
	...
	{	bpred [0] = pms->block [0] ;
		...
		if (bpred [0] >= 7)
			__psf_sprintf (psf, "MS ADPCM synchronisation error (%d).\n", bpred [0]) ;
...
{	MSADPCM_PRIVATE *pms ; 
	...
	total = __msadpcm_write (psf, pms, ptr, len) ;
...
{	MSADPCM_PRIVATE *pms ; 
	...
	{	writecount = (len >= bufferlen) ? bufferlen : len ;
		...
		count = __msadpcm_write (psf, pms, sptr, writecount) ;
		index += writecount ;
		total += count ;
		len -= writecount ;
		} ;
