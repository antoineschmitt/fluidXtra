...
static	int	__ima_read_block (SF_PRIVATE *psf, IMA_PRIVATE *pima) ;
static	int __ima_read (SF_PRIVATE *psf, IMA_PRIVATE *pima, short *ptr, int len) ;

static	int	__ima_write_block (SF_PRIVATE *psf, IMA_PRIVATE *pima) ;
static	int __ima_write (SF_PRIVATE *psf, IMA_PRIVATE *pima, short *ptr, int len) ;
...
{	IMA_PRIVATE		*pima ;
	...
	pimasize = sizeof (IMA_PRIVATE) + fmt->ima.blockalign + 3 * fmt->ima.channels * fmt->ima.samplesperblock ;

	if (! (psf->fdata = malloc (pimasize)))
		return SFE_MALLOC_FAILED ;
	pima = (IMA_PRIVATE*) psf->fdata ;
	memset (pima, 0, pimasize) ;
	
	pima->block   = (unsigned char*) pima->data ;
	pima->samples = (short*) (pima->data + fmt->ima.blockalign) ;
	
	pima->channels        = fmt->ima.channels ;
	pima->blocksize       = fmt->ima.blockalign ;
	pima->samplesperblock = fmt->ima.samplesperblock ;

	if (psf->datalength % pima->blocksize)
	{	__psf_sprintf (psf, "*** Warning : data chunk seems to be truncated.\n") ;
	...
	__ima_read_block (psf, pima) ;	/* Read first block. */
	
	return 0 ;
} /* __ima_reader_init */

static
int		__ima_read_block (SF_PRIVATE *psf, IMA_PRIVATE *pima)
{	int		chan, k, current, blockindex, index, indexstart ;
	...
	{	memset (pima->samples, 0, pima->samplesperblock * pima->channels) ;
		return 1 ;
		} ;
	...
	{	current = pima->block [chan*4] | (pima->block [chan*4+1] << 8) ;
		...
		if (pima->block [chan*4+3] != 0)
			__psf_sprintf (psf, "IMA ADPCM synchronisation error.\n") ;
		
		pima->samples [chan] = current ;

		/* __psf_sprintf (psf, "block %d : channel %d (current, index) : (%d, %d)\n", 
...
static
int __ima_read (SF_PRIVATE *psf, IMA_PRIVATE *pima, short *ptr, int len)
...
{	IMA_PRIVATE 	*pima ; 
	...
	total = __ima_read (psf, pima, ptr, len) ;
...
{	IMA_PRIVATE *pima ; 
	...
	{	readcount = (len >= bufferlen) ? bufferlen : len ;
		count = __ima_read (psf, pima, sptr, readcount) ;
...
{	IMA_PRIVATE		*pima ;
	...
	if (fmt->format != 0x0011)
		__psf_sprintf (psf, "*** Warning : format tag != WAVE_FORMAT_IMA_ADPCM.\n") ;
	
	fmt->ima.blockalign      = srate2blocksize (fmt->ima.samplerate) ;	
	fmt->ima.bitwidth        = 4 ;
	fmt->ima.extrabytes      = 2 ;
	fmt->ima.samplesperblock = 2 * (fmt->ima.blockalign - 4 * fmt->ima.channels) / fmt->ima.channels + 1 ;
...
static
int		__ima_write_block (SF_PRIVATE *psf, IMA_PRIVATE *pima)
{	int		chan, k, step, diff, vpdiff, blockindex, index, indexstart ;
	...
	if ((k = fwrite (pima->block, 1, pima->blocksize, psf->file)) != pima->blocksize)
		__psf_sprintf (psf, "*** Warning : short write (%d != %d).\n", k, pima->blocksize) ;
...
static
int __ima_write (SF_PRIVATE *psf, IMA_PRIVATE *pima, short *ptr, int len)
{	int		count, total = 0, index = 0 ;
...
{	IMA_PRIVATE 	*pima ; 
	...
	total = __ima_write (psf, pima, ptr, len) ;
...
{	IMA_PRIVATE *pima ; 
	...
	{	writecount = (len >= bufferlen) ? bufferlen : len ;
		...
		count = __ima_write (psf, pima, sptr, writecount) ;
		index += writecount ;
		total += count ;
		len -= writecount ;
		} ;
