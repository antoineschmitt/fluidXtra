...
typedef struct {
  ...
  DWORD frame_size;
  fluid_audio_stereoshort_func_t stereoshortfunc;
  void* stereoshortdata;
...
new_fluid_dsound_audio_driver(fluid_settings_t* settings, fluid_synth_t* synth)
{
  ...
  dev->stereoshortdata = NULL;  
  dev->stereoshortfunc = NULL;  
...
void fluid_dsound_audio_driver_setcallback(fluid_audio_driver_t* p, fluid_audio_stereoshort_func_t func, void* data) {
	
	fluid_dsound_audio_driver_t* dev = (fluid_dsound_audio_driver_t*) p;
	
	if (dev == NULL) return;
	
	dev->stereoshortdata = data;  
	dev->stereoshortfunc = func;  
	...
	ExitProcess(0);
      ...
      if (bytes1 > 0) {
		...
		cur_position += frames * dev->frame_size;
		  ...
		  // [AS MAY2012 Record]
		  if (dev->stereoshortfunc && dev->stereoshortdata) {
			  (*dev->stereoshortfunc)(dev->stereoshortdata, frames, buf1);
		  }
	  }
      ...
      if ((buf2 != NULL) && (bytes2 > 0)) {
		...
		cur_position += frames * dev->frame_size;
		  ...
		  // [AS MAY2012 Record]
		  if (dev->stereoshortfunc && dev->stereoshortdata) {
			  (*dev->stereoshortfunc)(dev->stereoshortdata, frames, buf2);
		  }
	  }
